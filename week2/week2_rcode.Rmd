...
The first step when preparing to analyze data is to read the data into R. 
...
```{r, eval=false}
setwd("/Users/jieun.shin/Documents/Teaching/MMC6936/week2")
```
...
The summarize function in dplyr provides a way to compute summary statistics with intutitive and readable code. 

```{r,eval=FALSE}
s <- summarize(dat, average=mean(dat$audience.size), standard_deviation=sd(dat$audience.size), median =median(dat$audience.size))
